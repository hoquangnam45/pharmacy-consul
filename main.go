...
func main() {
	...
	if consulServerUrl, ok := os.LookupEnv(ECS_CONSUL_SERVER_URL_ENV); ok {
		consulServers = h.Lift(ecs.ResolveHostModeService)(consulServerUrl).
			RetryUntilSuccess(time.Second*50, time.Second*10).
			PanicEval()
	}
	consulServers[advertiseIp] = true

	serverMode := env.GetEnvOrDefaultT("CONSUL_SERVER_MODE", strconv.ParseBool, true)
	bootstrapExpect := int(env.GetEnvOrDefaultT("CONSUL_SERVER_BOOTSTRAP_EXPECT", func(s string) (uint64, error) {
